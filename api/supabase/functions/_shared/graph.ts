...
export async function getValidGraphAccessToken(email: string): Promise<string> {
  ...
  if (error || !tokenRow) throw new Error("Outlook nicht verbunden.");

  const tokens = await decryptRow(tokenRow as Record<string, string>);
  const expMs = new Date(String(tokenRow.expires_at || "")).getTime();
