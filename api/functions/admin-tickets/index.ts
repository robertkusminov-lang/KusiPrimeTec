...
function normalizeRequestType(value: unknown): "direct" {
...
function requestTypeToAnfrageart(_value: "direct"): "direkt_einsatz" {
  return "direkt_einsatz";
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const runQuery = async (columns: string[], includeBucketColumn: boolean, includeRequestTypeColumn: boolean) => {
      ...
      if (includeBucketColumn && bucket !== "all") {
        query = query.eq("bucket", bucket);
      } else if (!includeBucketColumn && bucket === "archive") {
        query = query.in("status", ["Rapport_erstellt", "Storniert"]);
      } else if (!includeBucketColumn && bucket === "inbox") {
        query = query.eq("status", "Neu");
      } else if (!includeBucketColumn && bucket === "active") {
      ...
      if (requestTypeFilter) {
        if (includeRequestTypeColumn) {
          query = query.eq("request_type", "direct");
        } else if (
          requestTypeFilter === "direct" ||
          requestTypeFilter === "direkt_einsatz" ||
          requestTypeFilter === "direkt"
        ) {
          query = query.eq("anfrageart", "direkt_einsatz");
        }
      }
