...
describe("public SEO foundation", () => {
  it("publishes only indexable marketing routes in the sitemap", () => {
    ...
    expect(sitemap).not.toContain("/objektbetreuung-anfrage");
    expect(sitemap).not.toContain("/impressum");
  });

  it("keeps protected, transactional and legal routes out of the index", () => {
    ...
    expect(headers).toContain("/objektbetreuung-anfrage");
  ...
  it("redirects only routes confirmed as deprecated", () => {
    ...
    expect(redirects).toContain("/objektcheck /objektbetreuung 301");
