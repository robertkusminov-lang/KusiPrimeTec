...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 nav-glass">
      <div className="header-frame flex items-center justify-between gap-3 py-3.5 xl:gap-4">
        ...
        <div className="flex min-w-0 shrink-0 items-center gap-2">
          <a
            ...
            onClick={() => trackGoogleEvent("contact_phone_click", { placement: "header_desktop" })}
      ...
      <div id="mobile-navigation" className={clsx("header-frame overflow-hidden transition-all duration-200 xl:hidden", open ? "max-h-[calc(100vh-88px)] overflow-y-auto pb-3" : "max-h-0")}>
        <div className="premium-card grid gap-1 p-2">
          {mainNavItems.map((item) => (
            <LinkItem key={item.href} href={item.href} label={item.label} />
          ))}
          <LinkItem href={accountHref} label={accountLabel} />
          <a href="tel:+491776364393" onClick={() => trackGoogleEvent("contact_phone_click", { placement: "header_mobile" })} className="btn-secondary-premium header-chip mt-1 inline-flex items-center justify-center px-3 py-2 text-sm font-semibold">
          ...
          <a
            ...
            onClick={() => trackGoogleEvent("whatsapp_click", { placement: "header_mobile" })}
